...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    Or when invoked via Lambda URL:
    {
        "body": "{\"s3_bucket\": \"my-bucket\", \"s3_key\": \"path/to/document.pdf\", \"sections\": [\"all\"]}",
        ...other Lambda URL request fields...
    }
    ...
    try:
        ...
        # Check if this is a Lambda URL request by looking for the 'body' field
        # and other telltale signs of Lambda URL invocation
        if isinstance(event, dict) and 'body' in event and ('version' in event or 'requestContext' in event):
            logger.info("Detected Lambda URL invocation, extracting parameters from body")
            try:
                # Parse the body which contains our actual parameters
                if isinstance(event['body'], str):
                    body = json.loads(event['body'])
                else:
                    body = event['body']
                
                # Use the body as our new event
                event = body
                logger.info(f"Extracted parameters from body: {json.dumps(event)}")
            except Exception as e:
                logger.error(f"Failed to parse body: {str(e)}")
                return {
                    'statusCode': 400,
                    'body': json.dumps({
                        'error': f'Invalid request body format: {str(e)}'
                    })
                }
